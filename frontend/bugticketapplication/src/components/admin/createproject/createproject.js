...
function CreateProjectForm({ onSave, onClose, initialProjectData }) {
  ...
  const fetchDevelopers = async (page, size) => {
    ...
    try {
      let developersData;
      if (initialProjectData) {
        // Fetch all developers on the project and idle developers separately
        const projectDevelopers = initialProjectData.developers;
        const idleDevelopersResponse = await adminDeveloperService.getIdleDevelopers(page, size);
        developersData = {
          content: [...new Set([...projectDevelopers, ...idleDevelopersResponse.content])],
          totalElements: idleDevelopersResponse.totalElements + projectDevelopers.length
        };
      } else {
